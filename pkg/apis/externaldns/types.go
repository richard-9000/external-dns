...
func (cfg *Config) ParseFlags(args []string) error {
	...
	app.Flag("osb-endpoint", "When using the Opnsense Bind provider, specify the endpoint (default: osb-eu)").Default(defaultConfig.OSBEndpoint).StringVar(&cfg.OSBEndpoint)
	app.Flag("osb-api-rate-limit", "When using the Opnsense Bind provider, specify the API request rate limit, X operations by seconds (default: 20)").Default(strconv.Itoa(defaultConfig.OSBApiRateLimit)).IntVar(&cfg.OSBApiRateLimit)
